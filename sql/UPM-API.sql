...
CREATE TABLE `tbl_area` (
  ...
  `nfs_ip` varchar(15) NOT NULL COMMENT '区域所有NFS服务IP地址',
  `nfs_dir` varchar(128) NOT NULL COMMENT '区域所有NFS服务目录',
...
CREATE TABLE `tbl_cluster` (
  ...
  `relate_id` varchar(64) NOT NULL COMMENT '关联ID',
  `create_datetime` datetime NOT NULL COMMENT '创建时间',
  `create_user_login_name` varchar(32) NOT NULL COMMENT '创建者',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_definition_subserv` (
  ...
  `default_subserv_arch_definition_id` varchar(32) NOT NULL COMMENT '默认子服务架构定义ID，用于默认填充订单',
  `default_subserv_unit_scale_definition_id` varchar(32) NOT NULL COMMENT '默认子服务实例规模定义ID，用于默认填充订单',
  `major` bit(1) NOT NULL COMMENT '在服务中，是否为主要子服务\n0	fasle\n1	true',
  PRIMARY KEY (`id`),
  UNIQUE KEY `name_UNIQUE` (`name`)
...
CREATE TABLE `tbl_definition_subserv_arch` (
  ...
  `subserv_definition_id` varchar(32) NOT NULL COMMENT '所属子服务定义ID',
...
CREATE TABLE `tbl_definition_subserv_belongto_serv` (
  `id` varchar(32) NOT NULL COMMENT '主键',
  `subserv_definition_id` varchar(32) NOT NULL COMMENT '子服务定义ID',
  `serv_definition_id` varchar(32) NOT NULL COMMENT '所属服务定义ID',
  `priority` int(11) NOT NULL COMMENT '部署优先级',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_definition_subserv_relation` (
  ...
  `serv_definition_id` varchar(32) NOT NULL COMMENT '所属服务定义ID',
  `subserv_definition_id_from` varchar(32) DEFAULT NULL COMMENT '子服务关系起点',
  `subserv_definition_id_to` varchar(32) DEFAULT NULL COMMENT '子服务关系终点',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_definition_subserv_software` (
  `id` varchar(32) NOT NULL COMMENT '主键',
  `subserv_definition_id` varchar(32) NOT NULL COMMENT '所属子服务定义ID',
  `main_version` varchar(16) NOT NULL COMMENT '软件主版本',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_operate_log` (
  ...
  `operate_model_name` varchar(32) NOT NULL COMMENT '操作模块名称',
  `operate_action` varchar(32) NOT NULL COMMENT '操作动作',
  `operate_obj_id` varchar(32) NOT NULL COMMENT '操作对象ID',
  `operate_obj_name` varchar(128) NOT NULL COMMENT '操作对象名称',
  `operate_login_name` varchar(32) NOT NULL COMMENT '操作者',
  `operate_datetime` datetime NOT NULL COMMENT '操作时间',
  `operate_status` int(11) NOT NULL COMMENT '操作状态',
  PRIMARY KEY (`id`)
...
CREATE TABLE `tbl_order` (
  ...
  `create_user_login_name` varchar(32) NOT NULL COMMENT '创建者',
  PRIMARY KEY (`id`),
  KEY `serv_definition_id_INDEX` (`serv_definition_id`)
...
CREATE TABLE `tbl_subserv_software_image` (
  `id` varchar(32) NOT NULL COMMENT '主键',
  `subserv_software_definition_id` varchar(32) NOT NULL COMMENT '所属子服务软件定义ID',
  `fix_version` varchar(16) NOT NULL COMMENT 'fix版本',
  `site_id` varchar(32) NOT NULL COMMENT '所属站点ID',
  `relate_id` varchar(64) NOT NULL COMMENT '关联ID',
  `create_datetime` datetime NOT NULL COMMENT '创建时间',
  `create_user_login_name` varchar(32) NOT NULL COMMENT '创建者',
...
-- Dump completed on 2017-02-21 16:36:20
